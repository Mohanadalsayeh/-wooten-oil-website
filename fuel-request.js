...
export async function onRequestPost(context) {
  ...
  // Save the request first when a D1 binding named DB is configured.
  // This makes the order available even if email delivery later has a temporary problem.
  if (env.DB) {
    try {
      ...
      return json({ success: false, error: 'This request could not be saved. Please try again.' }, 500);
    }
  }
  ...
  const htmlRows = rows.map(([label, value]) => `
    <tr>
      <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;font-weight:700;vertical-align:top;width:210px">${esc(label)}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #e5e7eb;vertical-align:top">${esc(value)}</td>
    </tr>`).join('');

  const html = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#111827;background:#f3f4f6;padding:24px">
    <div style="max-width:700px;margin:auto;background:#fff;border-radius:12px;overflow:hidden">
      <div style="background:#b9342b;color:#fff;padding:20px 24px">
        <div style="font-size:22px;font-weight:800">New Fuel Delivery Request</div>
        <div style="margin-top:6px;font-size:16px">${esc(requestNumber)}</div>
      </div>
      <table role="presentation" style="width:100%;border-collapse:collapse">${htmlRows}</table>
      <div style="padding:16px 24px;color:#6b7280;font-size:12px">Submitted from ${esc(submittedFrom || 'wootenoil.com')}</div>
    </div>
  </body></html>`;
  ...
  const emailPayload = {
    ...
    tags: [{ name: 'request_number', value: requestNumber.replace(/[^A-Za-z0-9_-]/g, '_') }],
  };
  ...
  try {
    ...
    try { resendData = await resendResponse.json(); } catch {}
  } catch (error) {
    console.error('Resend request failed', error);
    if (env.DB) {
      await env.DB.prepare('UPDATE fuel_requests SET email_status = ? WHERE request_number = ?')
        .bind('network_error', requestNumber).run().catch(() => {});
    }
    return json({ success: false, error: 'The order was saved, but the email notification could not be sent. Please call Wooten Oil.' }, 502);
  }
  ...
  if (env.DB) {
    await env.DB.prepare('UPDATE fuel_requests SET email_status = ?, resend_email_id = ? WHERE request_number = ?')
      .bind('sent', resendData.id || '', requestNumber).run().catch(() => {});
  }

  return json({ success: true, requestNumber, emailId: resendData.id || null });
}

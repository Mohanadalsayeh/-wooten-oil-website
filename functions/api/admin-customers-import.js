...
export async function onRequestPost({ request, env }) {
  ...
  for (const row of customers) {
    ...
    const name = textValue(row?.account_name);
    ...
    batch.push(
      statement.bind(
        acct,
        name.slice(0, 200),
        textValue(row?.address1).slice(0, 250),
        textValue(row?.address2).slice(0, 250),
        textValue(row?.city).slice(0, 120),
        textValue(row?.state).slice(0, 30),
        textValue(row?.zip_code).slice(0, 20),
        textValue(row?.phone)
          .replace(/[^\d]/g, "")
          .slice(0, 30),
        textValue(row?.email).slice(0, 254)
      )
    );
  }

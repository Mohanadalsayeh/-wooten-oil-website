(function(){
  ...
  function showSuccess(requestNumber,warning){
    ...
    no.style.display='inline-flex';
    no.textContent='Done';
    if(status) status.textContent=warning||'Your fuel request is saved for Wooten Oil to review.';
  ...
  async function sendRequest(request){
    ...
    try{
      ...
      if(!response.ok || data.success!==true || data.saved!==true || data.requestNumber!==request.requestNumber){
        throw new Error(data.error || data.message || 'The server did not confirm that your request was saved. Keep this form open and try again, or call Wooten Oil at (901) 476-2684.');
      }
      showSuccess(data.requestNumber,data.warning||data.message||'');
    }catch(err){
      console.error('Fuel request submission failed:',err);
      showError(err.message||'The request was not confirmed as saved. Please try again.');
    }finally{

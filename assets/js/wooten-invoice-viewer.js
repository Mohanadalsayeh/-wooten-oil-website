...
if(!admin){
 ...
 nav.innerHTML='<button type="button" class="dashboard-back iv-dashboard-back" data-close aria-label="Back to Dashboard"><svg class="dashboard-back-chevron" viewBox="0 0 24 24" aria-hidden="true"><path d="m15 18-6-6 6-6"></path></svg><span>Dashboard</span></button><span class="iv-breadcrumb"><span aria-hidden="true">/</span> Open Invoices</span>';
 const heading=document.createElement('div');heading.className='iv-customer-heading';
 const identity=document.createElement('div');identity.append(title,account);
 heading.append(identity,list.querySelector('[data-refresh]'));
 head.append(brand,nav,close,heading);
 const footer=list.querySelector('.iv-foot');footer.replaceChildren();footer.textContent='Wooten Oil Co Inc. • Customer Portal';
}

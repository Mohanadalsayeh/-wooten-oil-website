...
async function exportPdf(){
 ...
 try{
  ...
  const blob=await window.WootenInvoicePdf.build('Open Invoices - Customer '+account,
   ...
   rows.map(r=>[r.invoice_no,r.invoice_type,r.division,date(r.invoice_date),date(r.due_date),amount(r.balance_cents/100)]));

(function(){
  ...
  const statusNames={approved:'Approved',pending:'Pending / Processing',declined:'Declined',canceled:'Canceled',unsubmitted:'Unsubmitted',expired:'Expired',failed:'Failed',review:'Review needed',unknown:'Unknown'};
  const providerNames={heartland:'Heartland',globalpayments:'Global Payments',unknown:'Unknown'};
  const environmentNames={sandbox:'Sandbox test',production:'Live',unknown:'Unknown'};
  ...
  function title(row){return row.status==='approved'?(row.environment==='sandbox'?'Sandbox Payment Confirmation':row.environment==='production'?'Payment Receipt':'Payment Transaction'):'Payment Status Record';}
  ...
  function render(data){
    ...
    get('ptSummary').innerHTML=[['total','All matches'],['approved','Approved'],['pending','Pending'],['declined','Declined'],['canceled','Canceled'],['review','Review needed']].map(([key,label])=>'<div><span>'+label+'</span><strong>'+Number(data.summary[key]||0).toLocaleString()+'</strong></div>').join('');
  ...
  async function openDetail(id,refresh=false){
    ...
    try{const data=await api(endpoint+'/'+encodeURIComponent(id));if(token!==detailSequence||modal.hidden)return;renderDetail(data.transaction);}
  ...
  function closeDetail(){detailSequence++;current=null;activeId='';if(!modal.hidden){modal.hidden=true;document.body.style.overflow=oldOverflow;opener?.focus();}get('ptDetail').innerHTML='';get('ptPrint').disabled=true;}

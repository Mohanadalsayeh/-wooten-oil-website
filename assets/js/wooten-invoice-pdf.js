...
function buildDetail(data){
 ...
 const header=()=>{ops+='0.83 0.15 0.19 rg 40 727 30 30 re f\n';ops+='BT /F2 11 Tf 1 1 1 rg 1 0 0 1 45 738 Tm (WO) Tj ET\n';text('WOOTEN OIL CO INC.',80,752,12,true);text('513 East Sanford Avenue',80,734,8);text('Covington, TN 38019',80,722,8);text('(901) 476-2684',80,710,8);text('support@wootenoil.com',80,698,8);wrapText(data.title||'Invoice',28,Infinity).forEach((part,i)=>rightText(part,752-i*13,12,true));if(data.number)rightText(data.number,733,16,true);rightText(String(data.meta).replace(/·/g,'-'),data.number?716:730,8);line(684,40,572,true);y=660;};
 const ensure=h=>{if(y-h<72){footer();header();}};
 header();text(data.customerLabel||'Customer',40,y,9);rightText(data.balanceLabel||'Invoice Remaining balance',y,9);y-=20;

...
async function adminCustomerActivityGet({request,env}){
  try{
    ...
    const [payments,documents,communications,fuelRequests,applications,logins,paymentChart,fuelChartSummary,lastChartPayment]=await Promise.all([
    ...
    return notificationJson({success:true,customer,payments,documents,communications,fuel_requests:fuelRequests,applications,login_activity:logins,payment_chart:paymentChart,payment_chart_last:lastChartPayment[0]||null,fuel_chart_summary:fuelChartSummary[0]||{},chart_months:chartMonths,pagination});

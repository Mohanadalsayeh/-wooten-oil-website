...
async function adminUsersApi({request,env}){
  ...
  }catch(error){console.error("adminUsersApi failed",error);const errorText=String(error?.message||"").toLowerCase();const duplicate=errorText.includes("unique")||errorText.includes("constraint failed")&&errorText.includes("username");const schema=errorText.includes("no such column")||errorText.includes("has no column");return notificationJson({success:false,error:duplicate?"That username is already in use.":schema?"The administrator database is being upgraded. Please refresh the page and try once more.":"The admin user could not be saved."},duplicate?409:500);}
}

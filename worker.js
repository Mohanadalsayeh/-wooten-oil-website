...
async function adminCustomerActivityGet({request,env}){
  try{
    ...
    const [payments,documents,communications,fuelRequests,applications,logins,paymentChart]=await Promise.all([
    ...
    return notificationJson({success:true,customer,payments,documents,communications,fuel_requests:fuelRequests,applications,login_activity:logins,payment_chart:paymentChart,pagination});

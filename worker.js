...
async function adminCustomerActivityGet({request,env}){
  try{
    ...
    const [payments,documents,communications,fuelRequests,applications,logins,paymentChart,fuelChartSummary]=await Promise.all([
      ...
      safeRows(env.DB.prepare(`WITH normalized AS (SELECT amount,CASE WHEN COALESCE(NULLIF(posting_date,''),NULLIF(deposit_date,''),payment_date) LIKE '____-__-__%' THEN substr(COALESCE(NULLIF(posting_date,''),NULLIF(deposit_date,''),payment_date),1,7) WHEN COALESCE(NULLIF(posting_date,''),NULLIF(deposit_date,''),payment_date) LIKE '__/__/____%' THEN substr(COALESCE(NULLIF(posting_date,''),NULLIF(deposit_date,''),payment_date),7,4)||'-'||substr(COALESCE(NULLIF(posting_date,''),NULLIF(deposit_date,''),payment_date),1,2) ELSE NULL END AS month FROM customer_payments WHERE account_number=?) SELECT month,SUM(COALESCE(amount,0)) AS total_amount,COUNT(*) AS payment_count FROM normalized WHERE month IS NOT NULL GROUP BY month ORDER BY month DESC LIMIT 12`).bind(account).all(),"payment chart"),
      safeRows(env.DB.prepare(`SELECT COUNT(*) AS request_count,SUM(CASE WHEN trim(COALESCE(gallons,'')) GLOB '[0-9]*' THEN CAST(REPLACE(gallons,',','') AS REAL) ELSE 0 END) AS total_gallons FROM fuel_requests WHERE datetime(received_at)>=datetime('now','-12 months') AND (customer_account_number=? OR ((customer_account_number IS NULL OR trim(customer_account_number)='') AND (lower(email)=lower(?) OR phone=?)))`).bind(account,String(customer.email||""),String(customer.phone||"")).all(),"fuel chart")
    ]);
    ...
    return notificationJson({success:true,customer,payments,documents,communications,fuel_requests:fuelRequests,applications,login_activity:logins,payment_chart:paymentChart,fuel_chart_summary:fuelChartSummary[0]||{},pagination});
